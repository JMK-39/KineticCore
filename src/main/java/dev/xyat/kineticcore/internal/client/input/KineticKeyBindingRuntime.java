...
public final class KineticKeyBindingRuntime {
    ...
    public static synchronized void initialize() {
        ...
        FMLJavaModLoadingContext.get().getModEventBus().addListener(KineticKeyBindingRuntime::onRegisterKeyMappings);
        MinecraftForge.EVENT_BUS.addListener(KineticKeyBindingRuntime::onClientTick);
        MinecraftForge.EVENT_BUS.addListener(KineticKeyBindingRuntime::onScreenKeyPressed);
        MinecraftForge.EVENT_BUS.addListener(KineticKeyBindingRuntime::onScreenMousePressed);
    }

    public static synchronized long register(KineticKeyBindings.Definition definition) {
        if (registrationFinished) {
            throw new IllegalStateException("Key bindings must be registered before RegisterKeyMappingsEvent completes: " + definition.translationKey());
        }
        if (IDS_BY_TRANSLATION_KEY.containsKey(definition.translationKey())) {
            throw new IllegalArgumentException("Duplicate key binding translation key: " + definition.translationKey());
        }

        long id = NEXT_ID.getAndIncrement();
        ENTRIES.put(id, new Entry(definition));
        IDS_BY_TRANSLATION_KEY.put(definition.translationKey(), id);
        return id;
    }
    ...
    private static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        for (Entry entry : snapshot()) {
            ...
            while (mapping.consumeClick()) {
                if (!entry.definition.enabledWhen().getAsBoolean()) continue;
                if (!modifiersMatch(entry.definition)) continue;
                entry.definition.onPressed().onPressed();
            }
        }
    }
    ...
    private static void handleScreenPress(InputConstants.Key input, CancelAction cancelAction) {
        for (Entry entry : snapshot()) {
            ...
            if (entry.definition.onPressed().onPressed()) {
    ...
    private static KeyMapping createMapping(KineticKeyBindings.Definition definition) {
        return new KeyMapping(
                definition.translationKey(),
                conflictContext(definition.context()),
                keyModifier(definition.modifier()),
                inputType(definition.device()),
                definition.keyCode(),
    ...
    private static InputConstants.Type inputType(KineticKeyBindings.Device device) {
        return switch (device) {
            case KEYBOARD -> InputConstants.Type.KEYSYM;
            case MOUSE -> InputConstants.Type.MOUSE;
        };
    }

    private static boolean modifiersMatch(KineticKeyBindings.Definition definition) {
        if (!definition.exactModifiers()) return true;
    ...
    private static final class Entry {
        private final KineticKeyBindings.Definition definition;
        private KeyMapping mapping;

        private Entry(KineticKeyBindings.Definition definition) {
